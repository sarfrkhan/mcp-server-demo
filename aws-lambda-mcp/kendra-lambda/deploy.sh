...
# Kendra MCP Server Lambda Deployment Script

# Configuration
FUNCTION_NAME="kendra-mcp-server"
REGION="us-east-1"  # Change this to your preferred region
ROLE_NAME="kendra-mcp-lambda-role"
POLICY_NAME="kendra-mcp-lambda-policy"

echo "Deploying Kendra MCP Server to AWS Lambda..."
...
create_zip('package', 'kendra-mcp-lambda.zip')
print('Created kendra-mcp-lambda.zip successfully')
"
...
# Create and attach Kendra access policy
...
aws lambda create-function \
    ...
    --zip-file fileb://kendra-mcp-lambda.zip \
    --timeout 30 \
    --memory-size 256 \
...
API_ID=$(aws apigateway create-rest-api \
    --name "kendra-mcp-api" \
    --description "API Gateway for Kendra MCP Server" \
    --region $REGION \
    --query 'id' \
    --output text 2>/dev/null || \
aws apigateway get-rest-apis \
    --query "items[?name=='kendra-mcp-api'].id" \
    --output text \
    --region $REGION)
...
aws apigateway get-resources \
    --rest-api-id $API_ID \
    --region $REGION \
    --query "items[?pathPart=='kendra-mcp'].id" \
    --output text)
...
# Get API endpoint
API_ENDPOINT="https://$API_ID.execute-api.$REGION.amazonaws.com/prod/kendra-mcp"
...
echo "3. Add server:"
echo "   - Server Name: Kendra MCP Server"
...
echo '}'
echo ""
echo "Note: Make sure to set KENDRA_INDEX_ID environment variable in the Lambda function"
echo "or provide indexId parameter when calling KendraQueryTool."

# Cleanup
rm -rf package kendra-mcp-lambda.zip
